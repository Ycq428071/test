  ...
  <div class="app-container">
    <el-form class="gl-search_form" :model="queryParams" ref="queryForm" :inline="true">
      ...
      <el-form-item label="所属大队" prop="deptId">
        <search-select v-model="queryParams.deptId" :listFn="listDeptFn" :backArr="backObj.dept" label="deptName"
    ...
    <div class="gl-body">
      ...
      <el-table height="calc(100vh - 84px - 60px - 86px - 70px - 56px - 50px)" v-loading="loading" :data="staffList"
        ...
        <el-table-column label="所属岗位" align="center" prop="positionName" width="250" />
        ...
        <el-table-column label="操作" align="center" fixed="right">
          <template slot-scope="scope">
            ...
            <router-link :to="'/securitymanagement/selfcomplementary/index/' + scope.row.securityId">
              <el-button type="text" size="mini">自补</el-button>
            </router-link>
            <el-button style="margin: 0 10px;" type="text" size="mini" @click="changeHistory(scope.row)"
    ...
    <el-dialog v-dialogDrag title="变更记录" :visible.sync="dialogTableVisible" :modal-append-to-body="false" width="80vw">
      <el-table :data="gridData">
        ...
        <el-table-column width="150" label="账号" align="center" prop="account" />
        ...
        <el-table-column width="150" label="服装款" align="center" prop="clothingStyle" />
...
<script>
import { listStaff, delStaff, changeHistory } from "@/api/SecurityManagement/Security.js";
import { exportAPI } from "@/api/SecurityManagement/supplementing.js";
...
export default {
  dicts: ['sys_user_sex', 'marriage', 'nation', 'education', 'politicaloutlook', 'discharge', 'health', 'qualification', 'securitylevel', 'recordproof', 'domicile', 'endowmentinsurance', 'retirementcertificate', 'accidentinsurance', 'clothingdeposit', 'joblevel'],
  name: "Supplementing",
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除保安人员管理编号为"' + securityIds + '"的数据项？').then(function () {
