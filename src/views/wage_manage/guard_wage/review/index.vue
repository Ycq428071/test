      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="allChecked"
            @click="handleUpdate"
            v-hasPermi="['salary:employee:check']"
          >复核</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            @click="handleDelete"
            v-hasPermi="['salary:employee:unCheck']"
          >消复核</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
...
<script>
import { listSalary, checkSalary, uncheckSalary, queryConfig } from "@/api/wage_manage/guard_wage"
...
export default {
  ...
  watch: {
    $route(route) {
      if(route.name === 'Wg_review') this.getList()
    },
  },
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.$modal.confirm('是否确认复核指定项？').then(function() {
        return checkSalary(ids);
      }).then(() => {
        this.getList();
    ...
    async handleDelete(row) {
      const ids = row.id || this.ids;
      this.$modal.confirm('是否确认消复核指定项？').then(function() {
        return uncheckSalary(ids);
      }).then(() => {
        this.getList();
        this.$modal.msgSuccess("消复核成功");
      }).catch(() => {});
    },
