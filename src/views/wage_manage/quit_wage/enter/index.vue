  ...
  <div class="app-container">
    ...
    <div class="gl-body">
      ...
      >
        ...
        <el-table-column label="离职日期" align="center" prop="resignationTime" show-overflow-tooltip />
      </el-table>
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              ...
              @click="$router.push('/wage_manage/quit_wage/qw_enter-add/index/' + queryParamsSecu.securityId + '/0')"
            >
              新增
            </el-button>
          </el-col>
        ...
        <el-table-column label="操作" align="center" fixed="right" width="150">
          <template slot-scope="scope">
            <el-button
              ...
              @click="$router.push('/wage_manage/quit_wage/qw_enter-add/index/' + queryParamsSecu.securityId + '/' + scope.row.id + '?row=' + JSON.stringify(scope.row))"
            >
              修改
            </el-button>
            <el-button
            ...
            <el-button
              ...
              type="text"
              v-hasPermi="['salary:employee:add']"
              @click="$router.push('/wage_manage/quit_wage/qw_enter-add/index/' + queryParamsSecu.securityId + '/0' + '?row=' + JSON.stringify(scope.row))"
            >
              工资复制
            </el-button>
          </template>
...
<script>
import { listQuitGuard } from "@/api/wage_manage/quit_wage"
...
import { listStream, delSalary } from "@/api/wage_manage/guard_wage"
import { enterConfig } from "@/api/wage_manage/quit_wage"
...
export default {
  ...
  watch: {
    $route(route) {
      if(route.name === 'Wq_enter') this.getList()
    },
  },
  created() {
    this.getList()
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listQuitGuard(this.queryParams).then(response => {
