<template>
  <div class="app-container">
    ...
    <div class="gl-body">
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <el-button
            ...
            v-hasPermi="['salary:employee:salaryDetailsExport']"
          >导出工资明细</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            icon="el-icon-download"
            @click="handleExport"
            v-hasPermi="['salary:employee:exportGroupSalary']"
          >导出</el-button>
        </el-col>
      </el-row>
      ...
      >
        <!-- <el-table-column label="合同编号" align="center" prop="" show-overflow-tooltip />
        <el-table-column label="合同名称" align="center" prop="" show-overflow-tooltip /> -->
...
<script>
import { listPointWage } from "@/api/wage_manage/overall_query"
import { listDept, listUnit, listCont } from "@/api/wage_manage/query"
import { listPost } from '@/api/wage_manage/guard_manage'
import searchSelect from '@/views/components/searchSelect.vue'
import dayjs from 'dayjs'

export default {
  name: 'Guard_point_wage_query',
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listPointWage(this.queryParams).then(response => {
    ...
    handleExport() {
      ...
      this.download('/salary/employee/exportGroupSalary', {
        ...exportParams
      }, `保安驻点工资表_${new Date().getTime()}.xlsx`)
    },
    handleExportDetail() {
      const exportParams = {}
      for(let k in this.queryParams) {
      ...
      this.download('/salary/employee/salaryDetailsExport', {
        ...exportParams
      }, `保安工资合计明细表_${new Date().getTime()}.xlsx`)
    },
  }
}
</script>
